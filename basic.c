...
/*
 ...
 Compile the Program: The C program needs to be compiled into machine-readable code using a compiler (e.g., GCC).
 ...
 sol:- In dynamic memory allocation, memory is allocated at runtime during program execution.
  When we use static arrays, their size is fixed at the time of declaration and cannot be changed later.
  However, with dynamic memory allocation, we define an initial size, but we have the flexibility to resize it
  later using realloc() as per our requirements.

 In C, dynamic memory allocation functions from the stdlib.h library allow us to allocate memory at runtime.
 The primary functions used are:malloc , calloc , realloc , free
 ...
 Q13  Mathematical Functions and Type Conversions in Expressions
    ...
    2️⃣ Type Conversions in Expressions
   C allows implicit and explicit (type casting) conversions while performing operations on different data types.

     Implicit Type Conversion (Type Promotion)
     C automatically converts smaller data types to larger ones in an expression.

     Example: int → float in arithmetic operations.
    
     int a = 5;
     float b = 2.5;
     float result = a + b; // 'a' is promoted to float
     printf("Result: %.2f", result); // Output: 7.50
     Explicit Type Conversion (Type Casting)
     You can manually convert data types using type casting.
     
     int x = 10, y = 3;
     float division = (float)x / y; // Explicitly casting x to float
     printf("Division result: %.2f", division); // Output: 3.33


    Q14 what is ANCI?
    sol:- ANSI C stands for American National Standards Institute C, and its main functions are:
